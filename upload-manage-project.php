<!DOCTYPE html>
<html>
<head>
  <?php include 'source.php' ?>
</head>

<body>
  <?php include "nav.php"; ?>
  <div class="container-fluid" style=" background-color:#fff; height:100px; margin-top:50px;">
  </div>
  
  <div class="upload-upload-form col-kzmd-offset-0 col-md-12" style="background-color:#fff; margin-top:15px; padding-top:25px;">

  <div class="row col-md-offset-1 col-md-10" style="margin-top:55px;">

      <div class="col-md-10">
        
      </div>

      <div class="col-sm-6 col-md-3" >
        <div class="thumbnail col-md-12">
          <img data-src="holder.js/100%x200" alt="100%x200" src="data:image/svg+xml;base64," data-holder-rendered="true" style="height: 200px; width: 100%; display: block;">
          <div class="caption">
            <h4><strong>Project's Name</strong></h4>
            <p>Category</p>
            <p><a href="#" class="btn btn-default" role="button">Edit</a> <a href="#" class="btn btn-danger" role="button">Delete</a></p>
          </div>
        </div>
      </div>

      <div class="col-sm-6 col-md-3" >
        <div class="thumbnail col-md-12">
          <img data-src="holder.js/100%x200" alt="100%x200" src="data:image/svg+xml;base64," data-holder-rendered="true" style="height: 200px; width: 100%; display: block;">
          <div class="caption">
            <h4><strong>Project's Name</strong></h4>
            <p>Category</p>
            <p><a href="#" class="btn btn-default" role="button">Edit</a> <a href="#" class="btn btn-danger" role="button">Delete</a></p>
          </div>
        </div>
      </div>

      <div class="col-sm-6 col-md-3" >
        <div class="thumbnail col-md-12">
          <img data-src="holder.js/100%x200" alt="100%x200" src="data:image/svg+xml;base64," data-holder-rendered="true" style="height: 200px; width: 100%; display: block;">
          <div class="caption">
            <h4><strong>Project's Name</strong></h4>
            <p>Category</p>
            <p><a href="#" class="btn btn-default" role="button">Edit</a> <a href="#" class="btn btn-danger" role="button">Delete</a></p>
          </div>
        </div>
      </div>

      <div class="col-sm-6 col-md-3" >
        <div class="thumbnail col-md-12">
          <img data-src="holder.js/100%x200" alt="100%x200" src="data:image/svg+xml;base64," data-holder-rendered="true" style="height: 200px; width: 100%; display: block;">
          <div class="caption">
            <h4><strong>Project's Name</strong></h4>
            <p>Category</p>
            <p><a href="#" class="btn btn-default" role="button">Edit</a> <a href="#" class="btn btn-danger" role="button">Delete</a></p>
          </div>
        </div>
      </div>
    </div>

  </div>

      







      
</body>

</html>
